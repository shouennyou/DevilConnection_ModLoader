...
const ModManagerApi = {
	...
	init() {
		...
		Logger.info(`模组目录: ${this.pluginDir}`);
		this.ensureModOrder();
	},
	...
	scanPlugins() {
		...
		mods.sort((a, b) => {
			// 游戏核心归档始终保持最低优先级.
			const sa = SYSTEM_ASARS.includes(a.file) ? 1 : 0;
			const sb = SYSTEM_ASARS.includes(b.file) ? 1 : 0;
	...
	/** 读取 modorder.json, 不存在或解析失败时返回空数组. */
	readModOrder() {
	...
	writeModOrder(entries) {
		...
		try {
			...
			Logger.info(`已写入 ${MODORDER_FILE}`);
			return true;
		} catch (error) {
			Logger.error(`写入 ${MODORDER_FILE} 失败`, error);
			return false;
		}
	},
	...
	async scanModInfos() {
		...
		for (const mod of scanned) {
			// 游戏核心归档不属于模组, 不读取元信息.
			if (SYSTEM_ASARS.includes(mod.file)) continue;
	...
	/** 使用 original-fs 检测 mods 中游戏核心归档是否存在. */
	checkGameResources() {
		return {
			appAsar: SYSTEM_ASARS[0] && originalFs.existsSync(path.join(this.pluginDir, SYSTEM_ASARS[0])),
			appBakAsar: SYSTEM_ASARS[1] && originalFs.existsSync(path.join(this.pluginDir, SYSTEM_ASARS[1])),
		};
	},
 ...
 * 更新模组顺序与启用状态.
	...
	setModOrder(orderedMods) {
		...
		// 沿用现有配置中的游戏核心归档启用状态.
		const existingMap = new Map(this.readModOrder().map(e => [e.file, e]));

		// 普通模组按传入顺序连续编号.
		const normal = orderedMods
			.filter(e => e && this.isPluginEntryName(e.file) && !SYSTEM_ASARS.includes(e.file))
			.map((e, i) => ({ file: e.file, order: i + 1, enabled: e.enabled !== false }));

		// 扫描实际存在的游戏核心归档并追加到末尾.
		const systemEntries = [];
		for (const name of SYSTEM_ASARS) {
			if (originalFs.existsSync(path.join(this.pluginDir, name))) {
				const prev = existingMap.get(name);
				systemEntries.push({
					file: name,
					order: normal.length + systemEntries.length + 1,
					enabled: prev ? prev.enabled !== false : true,
				});
			}
		}

		return this.writeModOrder([...normal, ...systemEntries]);
	},
 ...
 * 使用 Electron net 以应用系统和会话代理, 最多跟随 5 次重定向.
	...
	/** 游戏存档目录, 位于 resourcesPath 的上一级. */
	getStorageDir() {
		return path.normalize(path.join(this.resourcesPath, '..', STORAGE_DIR));
	},

	/** 备份目录. */
	getBackupDir() {
		return path.join(this.resourcesPath, BACKUP_DIR);
	},
 ...
 * 将 _storage 下的 .sav 存档压缩为备份.
	...
	createBackup(name) {
		...
		if (!originalFs.existsSync(storageDir)) {
			return { success: false, message: '存档目录不存在' };
		}
		...
		try {
			...
			if (count === 0) {
				if (originalFs.existsSync(zipPath)) originalFs.unlinkSync(zipPath);
