// preload_extra.js - Extra preload API
const {
	ipcRenderer
} = require('electron');

// ─── ModManager CRUD ──────────────────────────────────────────────────────────
// 所有路径均相对于 process.resourcesPath
...
const modManagerAPI = {
	...
	/** 更新模组顺序与启用状态(系统文件始终排末尾),返回成功 boolean */
	...
	/** 检测游戏核心资源归档是否存在,返回 {appAsar, appBakAsar} */
	checkGameResources: async () => {
		return await ipcRenderer.invoke('modmanager:checkGameResources');
	},
 ...
 * IPC 只序列化普通属性,fs.Stats 的 isFile()/isDirectory() 等方法会丢失;
